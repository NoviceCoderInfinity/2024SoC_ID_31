...
def test(model, dataloader, device):
    ...
    with torch.no_grad():
        for i, (data, _) in enumerate(dataloader):
            ...
            x_hat, mu1, logvar1, mu2, logvar2 = model(data)  # Adjusted to match model output
            loss = F.binary_cross_entropy(x_hat, data, reduction='sum') + \
                   -0.5 * torch.sum(1 + logvar1 - mu1.pow(2) - logvar1.exp()) + \
                   -0.5 * torch.sum(1 + logvar2 - mu2.pow(2) - logvar2.exp())
            test_loss += loss.item()
